...
def index(request):
    ...
    if request.method == 'POST':
        if 'file' in request.POST:
            ...
            d = dates(date = date_time,filename=request.POST['filename'])
            d.save()
    ...
    if len(last_outputs.objects.all()) > 0:
        recent_uploaded_filename = dates.objects.last().filename
        recent_uploaded_time = dates.objects.last().date
        display_outputs = last_outputs.objects.last().args
    else:
        recent_uploaded_filename = ""
        display_outputs = [{},{},{},{}]
        recent_uploaded_time = ""
    ...
    return render(request, 'rstapp/index.html',
        ...
        'recent_uploaded_filename':recent_uploaded_filename,
        'recent_uploaded_time':recent_uploaded_time}
    )
